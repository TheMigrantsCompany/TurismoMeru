...
const BookingCard = ({ id_Service, price }) => {
  ...
  const [quantities, setQuantities] = useState({ adultos: 0, menores: 0, jubilados: 0 });
  ...
  useEffect(() => {
    if (excursion) {
      const total =
        ...
        excursion.price * (1 - (excursion.discountForMinors || 0) / 100) * quantities.menores +
        excursion.price * (1 - (excursion.discountForSeniors || 0) / 100) * quantities.jubilados;
      setTotalPrice(total);
    }
  ...
  return (
    <div className="bg-[#dac9aa] text-[#152917] p-4 rounded-lg max-w-sm shadow-lg border-2 border-[#425a66]">
      ...
      <div className="mb-3">
        ...
        {[{ type: "adultos", label: "Adultos" }, { type: "menores", label: "Menores" }, { type: "jubilados", label: "Jubilados" }].map(({ type, label }) => (
          <div key={type} className="flex items-center justify-between mb-1 text-sm">
            <span>{label}</span>
            <div className="flex items-center space-x-2">
              <button className="bg-[#f4a25b] px-2 rounded" onClick={() => handleQuantityChange(type, "decrement")}>-</button>
              <span>{quantities[type]}</span>
              <button className="bg-[#f4a25b] px-2 rounded" onClick={() => handleQuantityChange(type, "increment")}>+</button>
            </div>
          </div>
        ))}
      </div>

      <button className="bg-[#425a66] text-white w-full py-1 rounded text-sm hover:bg-[#152917]" onClick={handleAddToCart}>
        Añadir al carrito
      </button>
